...
def benchmark(func, height, root, repeat=5):
    """Возвращает среднее время выполнения func(n)"""
    times = timeit.repeat(lambda: func, repeat=repeat)
    return min(times)
...
def main():
    ...
    for height in test_data:
        res_recursive.append(benchmark(build_tree_recursive, height , root, repeat=10))
        res_iterative.append(benchmark(build_tree_iteractive, height , root, repeat=10))
